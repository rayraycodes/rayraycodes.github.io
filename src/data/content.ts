    ...
    "stories": [
      {
        ...
        "content": {
          "description": [
            "I build software for a person who does not exist. So do most of the people I work with. We picture a tidy human, one who answers messages promptly, rates things honestly from one to five, holds still long enough to be measured, and walks toward a goal in a straight line. We design for that person on purpose. And that person never shows up, because we are the ones who made them up.",
            "The real human is the one I actually am. I hold a grudge over a text left on read. I buy the running shoes and skip the run. I say I am fine and mean six other things. We love people who are bad for us, cry at commercials, refresh a thread for a message we already know has not arrived. None of this is a malfunction. This is us, working exactly as designed. And the most expensive mistakes I have watched us make with technology all begin the same way: we treat that human as a bug to be fixed.",
            ...
            "We decided words alone were enough, and stripped the tone out of everything we typed. Then in 1982, Scott Fahlman watched jokes land as insults on a message board, because we had amputated the voice from our own sentences. His fix was three characters, the first smiley. Think about what we did in that moment. We had deleted feeling from the medium, and within hours we were smuggling it back in by hand. We have never stopped. We turned emoticons into emoji, emoji into reactions, then GIFs, then voice notes, then 'lol' used not as laughter but as a cushion we slide under a sentence so it does not bruise.",
            {
              "type": "figure",
            ...
            "That timeline is us, not some company. No executive decided text needed feeling. Hundreds of millions of us decided it, one workaround at a time. When we keep hacking our own humanity back into the things we make, we are filing the most honest bug report there is, and it is written against our own blueprint.",
            "We decided work was predictable. We wrote the Waterfall method, specified the whole system up front, and marched toward it like a factory order, as if the people building it would not learn, change their minds, find the spec wrong, or burn out by month seven. We killed projects by the thousand. Then we wrote the Agile manifesto, and we like to call it a process change, but read it again: individuals and interactions over processes and tools, responding to change over following a plan. That is not management language. That is a room full of us admitting we are moody, learning, talking creatures, and that any plan pretending otherwise will lose.",
            "We decided connection was a number, and this is the one that breaks my heart, because we started from something true. We knew people crave each other. Then we handed that craving to an optimizer and let it choose the cheapest proxy. We turned connection into engagement. We turned friendship into a count. We turned being known into being seen, then into being measured. We built the like button and watched it convert affection into a metric. We tuned the feed and learned that our outrage holds our own gaze longer than our tenderness does. We built a connection machine and ended up lonelier than any generation before us.",
            {
              "type": "quote",
            ...
            "We decided love was a matching problem. We sorted ourselves by height, filtered by hobby, swiped through each other like a catalog. And then we fell in love the way we always have: by accident, bad timing, a laugh at the wrong moment, someone who was not our type at all. We optimized the search and starved the serendipity, then wondered why we describe dating like a second job. We knew, somewhere, that love was never a retrieval task. It is a story, and we cut out the part where stories need the unplanned.",
            "We decided we were dashboards. We strapped on the steps, the streaks, the sleep scores, the ring that grades the night and the app that scolds the sitting. We told ourselves that to measure is to know. But a streak has never once been a reason to live, and I have watched a sleep score make someone anxious enough to sleep worse. The number tells us what we did. It cannot tell us what it meant, and meaning is the only fuel we run on for long.",
            "Lay these side by side and the pattern is embarrassing, because it is always us. Every time, we modeled the thin slice of ourselves that fits in a database and threw the rest away as noise. But the rest is not noise. Our drama is how we flag what matters. Our flaws are where our stories live. Our feelings are the highest-quality data we produce, direct reports on value and threat and longing that no sensor catches. Our need for each other is not a feature request, it is the load-bearing wall of a life. Every time we strip those out, we do not get a simpler human. We lose the human and keep the paperwork.",
            {
              "type": "quote",
              "text": "Strip out the mess and we do not get a simpler human. We lose the human and keep the paperwork."
            },
            "Which brings me to where we are standing right now, with AI, holding the most flattering mirror we have ever made. I want to be precise about the danger, because it is not the model. The temptation lives in us, and it comes in two flavors. The crude one treats people as engagement to be maximized, and we already know how that ends. The subtle one is worse, because it sounds wise: it says feeling matters, so let us track it, score the mood, optimize the emotions too. That is not us honoring the human. That is us pointing the fitness tracker at the heart.",
            "Here is what the whole history has been trying to tell me. Every correction worked the same way, because a human made room for the mess instead of cleaning it up. We made room for intuition with the interface. We made room for tone with the smiley. We made room for changing our minds with Agile. So the work in front of us is not to add an emotions module. It is to change how we think, and finally build for the creature we actually are: the one who needs to be understood more than analyzed, who heals in relationships and not in dashboards, whose flaws are not errors to patch but the texture of a person. If we build AI to read our signals so a machine can manage us, that is only our oldest mistake in a newer costume. If we build it to help the people who love us understand us, and then step out of the way, that is the first time we have read the whole book."
          ],
          "work": [],
          "impact": "We keep building for the tidy human and getting corrected by the real one. Forty years of smileys and workarounds and abandoned dashboards are us, patiently arguing with our own assumptions: I am not noise. The machine was never the problem, and it was never the cure. It is a mirror. The most useful thing I can do as someone who builds is to change what I think a human is, and then believe us the first time.",
          "images": [
            {
